...
async def get_query_embedding(question: str) -> list[float] | None:
    ...
    try:
        ...
        logger.warning("Cohere query embedding failed: %s", exc)
    return None
    ...
    *,
    ...
    if query_embedding:
        try:
            ...
            logger.warning(
                "Semantic lookup failed; using keyword search", exc_info=True
            )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                return AskNotesResponse(
                    answer="The answer provider is temporarily unavailable. Matched notes:\n\n"
        ...
        logger.warning("Groq answer request failed: %s", exc)
        return AskNotesResponse(
            answer="The answer provider is temporarily unavailable. Matched notes:\n\n"
            + "\n\n".join([f"**{n.title}**: {n.content}" for n in top_notes]),
            sources=sources,

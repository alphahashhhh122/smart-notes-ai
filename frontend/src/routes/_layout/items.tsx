...
interface Note { id: string; title: string; content: string; tags?: string | null; summary?: string | null }
interface Version { id: string; note_id: string; title: string; content: string; tags: string | null; created_at: string }
...
const s: Record<string, React.CSSProperties> = {
  ...
  summaryBadge: { display: "inline-flex", alignItems: "center", gap: "3px", padding: "2px 7px", background: "rgba(52,211,153,0.1)", border: "1px solid rgba(52,211,153,0.15)", borderRadius: "5px", fontSize: "10px", color: "#34D399", marginBottom: "6px" },
  ...
  overlay: { position: "fixed" as const, inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" },
  ...
  shareBtn: { height: "34px", padding: "0 14px", background: "rgba(124,106,247,0.1)", color: "#9B8CF9", border: "1px solid rgba(124,106,247,0.2)", borderRadius: "8px", fontSize: "12px", fontWeight: 500, cursor: "pointer", display: "flex", alignItems: "center", gap: "6px" },
  ...
  versionItem: { padding: "12px", background: "#0D0E14", borderRadius: "8px", marginBottom: "8px", border: "1px solid #1A1B26" },
  versionTitle: { fontSize: "12px", fontWeight: 600, color: "#E8E8F0", margin: "0 0 4px" },
  versionMeta: { fontSize: "10px", color: "#4A4A6A", margin: "0 0 8px" },
  versionContent: { fontSize: "11px", color: "#7878A0", margin: 0, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const },
  restoreBtn: { marginTop: "8px", height: "28px", padding: "0 12px", background: "rgba(124,106,247,0.1)", color: "#9B8CF9", border: "1px solid rgba(124,106,247,0.2)", borderRadius: "6px", fontSize: "11px", cursor: "pointer" },
}
...
function AISummaryBox({ note }: { note: Note }) {
  ...
  return (
    <button style={{ ...s.actionBtn, width: "auto", padding: "0 8px", fontSize: "10px", gap: "3px", display: "flex", alignItems: "center", color: loading ? "#34D399" : "#7878A0" }} onClick={fetchSummary}>
...
function NoteCard({ note, onEdit, onDelete, onShare, onHistory }: {
  note: Note; onEdit: (n: Note) => void; onDelete: (id: string) => void
  onShare: (n: Note) => void; onHistory: (n: Note) => void
}) {
  ...
  return (
    <div style={s.card} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <div style={{ ...s.actions, opacity: hovered ? 1 : 0, transition: "opacity 0.15s" }}>
        <button style={s.actionBtn} onClick={e => { e.stopPropagation(); onHistory(note) }} title="Version history">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
        </button>
        <button style={s.actionBtn} onClick={e => { e.stopPropagation(); onShare(note) }} title="Share">
...
function VersionModal({ note, onClose }: { note: Note; onClose: () => void }) {
  ...
  const restore = async (versionId: string) => {
    const res = await fetch(`${BACKEND}/api/v1/notes/${note.id}/restore/${versionId}`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` }
    })
    if (res.ok) {
  ...
  return (
    <div style={s.overlay} onClick={onClose}>
      <div style={{ ...s.modal, maxWidth: "480px" }} onClick={e => e.stopPropagation()}>
        ...
        {isLoading ? (
          <p style={{ color: "#7878A0", fontSize: "13px" }}>Loading versions...</p>
        ) : versions.length === 0 ? (
          <p style={{ color: "#7878A0", fontSize: "13px" }}>No previous versions. Versions are saved when you edit a note.</p>
        ) : (
          versions.map(v => (
            <div key={v.id} style={s.versionItem}>
              <p style={s.versionTitle}>{v.title}</p>
              <p style={s.versionMeta}>{new Date(v.created_at).toLocaleString()}</p>
              <p style={s.versionContent}>{v.content}</p>
              <button style={s.restoreBtn} onClick={() => restore(v.id)}>Restore this version</button>
            </div>
          ))
        )}
      </div>
    ...
    <div style={s.overlay} onClick={onClose}>
      <div style={{ ...s.modal, maxWidth: "420px" }} onClick={e => e.stopPropagation()}>
        ...
        <p style={{ fontSize: "13px", color: "#7878A0", marginBottom: "16px" }}>
          Create a public read-only link for "<strong style={{ color: "#E8E8F0" }}>{note.title}</strong>"
        </p>
        {!shareUrl ? (
          <button style={{ ...s.saveBtn, opacity: loading ? 0.6 : 1 }} onClick={createLink} disabled={loading}>
            {loading ? "Generating..." : "Generate Share Link"}
          </button>
        ) : (
          <div>
            <div style={{ background: "#0D0E14", border: "1px solid #1A1B26", borderRadius: "8px", padding: "10px 12px", fontSize: "12px", color: "#9B8CF9", wordBreak: "break-all" as const, marginBottom: "12px" }}>
              {shareUrl}
            </div>
            <button style={{ ...s.saveBtn, background: copied ? "#34D399" : "#7C6AF7" }} onClick={copy}>
  ...
  const suggestTags = async () => {
    ...
    try {
      const res = await AiService.askNotes({
        requestBody: { question: `Suggest exactly 3 short comma-separated tags for a note titled "${title}" with content: ${content.slice(0, 200)}. Reply with only the tags.` }
      })
...
function NotesPage() {
  ...
  return (
    <div style={s.page}>
      ...
      {historyNote && <VersionModal note={historyNote} onClose={() => setHistoryNote(null)} />}
    </div>
  )
}
